...
export default function CartPage() {
  // 👈 Fix: useSelector ko stable rakha hai
  ...
  const itemCount = useMemo(
    () => (items || []).reduce((a, c) => a + Number(c.qty || 0), 0),
    [items]
  );
        ...
        <div className="lg:col-span-8">
          {items.length === 0 ? (
            ...
            <div className="space-y-6">
              {items.map((i) => (
                // ⭐️ Item Card
                <div key={`${i.product}-${i.size}-${i.color}`} className="flex gap-4 p-4 border rounded-lg items-start dark:border-zinc-700 dark:bg-zinc-800">
                  <Link to={`/product/${i.product}`} className="shrink-0 w-28 h-32 rounded-md overflow-hidden bg-gray-100 dark:bg-zinc-700">
                    <img
                  ...
                  <div className="flex-1 min-w-0">
                    <Link to={`/product/${i.product}`} className="block text-lg font-semibold text-gray-900 dark:text-white hover:underline">
                      {i.title}
                    </Link>
                    <div className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                      {i.size && <span className="mr-3">Size: <strong className="text-gray-700 dark:text-gray-200">{i.size}</strong></span>}
                      {i.color && <span>Color: <strong style={{ textTransform: 'capitalize' }} className="text-gray-700 dark:text-gray-200">{i.color}</strong></span>}
                    </div>
                    <div className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                      Unit Price: ₹{Number(i.price).toFixed(2)}
                    </div>
                    
                    {/* ⭐️ Quantity Controller */}
                    <div className="flex items-center justify-between mt-4">
                      <div className="flex items-center border rounded-md h-10 dark:border-zinc-700">
                        ...
                        <span className="px-4 text-base font-semibold dark:text-white">
                          {i.qty}
                        </span>
                        <button
